...
rm(list = ls())
setwd("F:\\MFDM\\ZexuChen\\School Roster LEAP 3-8\\Science1")
list_sc <- list.files( pattern = ".{3}SCIRosterLEAP.pdf")
...
df_sc1 <- function(raw_sc){
            ...
            GRADE <- substring(str_extract(raw_sc, "Spring 2019\r\nGrade:\\s[A-Z]?\\d+"),
                                 nchar("Spring 2019\r\nGrade:")+1)
            ...
            INV_S <- as.numeric(str_trim(substr(raw_sc,regexpr("Strong \\(S\\)", raw_sc)+nchar("Strong \\(S\\)"), 
	        	...
	        	EVA_S <- as.numeric(str_trim(substr(raw_sc,regexpr("Strong \\(S\\)", raw_sc)+
		                                       nchar("Strong \\(S\\)")+30, regexpr("Strong \\(S\\)", raw_sc)+ nchar("Strong \\(S\\)")+45)))
	        	REA_S <- as.numeric(str_trim(substr(raw_sc,regexpr("Strong \\(S\\)", raw_sc)+
            ...
            INV_M <- as.numeric(str_trim(substr(raw_sc,regexpr("Moderate \\(M\\)", raw_sc)+nchar("Moderate \\(M\\)"), regexpr("Moderate \\(M\\)", raw_sc)+ 
                                                  nchar("Moderate \\(M\\)")+30)))
	        	EVA_M <- as.numeric(str_trim(substr(raw_sc,regexpr("Moderate \\(M\\)", raw_sc)+
		                                       nchar("Moderate \\(M\\)")+30, regexpr("Moderate \\(M\\)", raw_sc)+ nchar("Moderate \\(M\\)")+45)))
            REA_M <- as.numeric(str_trim(substr(raw_sc,regexpr("Moderate \\(M\\)", raw_sc)+nchar("Moderate \\(M\\)")+45, regexpr("Moderate \\(M\\)", raw_sc)+ 
                                                   nchar("Moderate \\(M\\)")+65)))
           
		

            INV_W <- as.numeric(str_trim(substr(raw_sc,regexpr("Weak \\(W\\)", raw_sc)+nchar("Weak \\(W\\)"), regexpr("Weak \\(W\\)", raw_sc)+ nchar("Weak \\(W\\)")+30)))
	        	EVA_W <- as.numeric(str_trim(substr(raw_sc,regexpr("Weak \\(W\\)", raw_sc)+nchar("Weak \\(W\\)")+30, regexpr("Weak \\(W\\)", raw_sc)+ nchar("Weak \\(W\\)")+45)))
            REA_W <- as.numeric(str_trim(substr(raw_sc,regexpr("Weak \\(W\\)", raw_sc)+nchar("Weak \\(W\\)")+45, regexpr("Weak \\(W\\)", raw_sc)+ nchar("Weak \\(W\\)")+65)))
...
write.csv(sch_sc_v1_1st_r, 'dist_sch_sc_drc.csv', row.names=FALSE)
  ...
  ################### extract students without names    ##################
    ...
    df_sc3 <- function(raw_sc){
            ...
            DISTRICTCODE <- str_trim(substring(str_extract(raw_sc, "School System:\\s\\S+"),
  	        ...
  	        DISTRICTNAME <- str_trim(substring(str_extract(raw_sc, "School System:\\s.\\S+.+"),
  	                                         nchar("School System: ")+1+nchar(DISTRICTCODE)+1))
...
```{r}      
     ...
     std_sc_noname_long <- data.frame(
          ...
          Evaluate = str_trim(substr(TOGETHER_NM,125,135)),
          ReasonScientifically = str_trim(substr(TOGETHER_NM,135,145))
...
setwd('F:\\MFDM\\ZexuChen\\School Roster LEAP 3-8\\R')
write.csv( std_sc_drc, "dist_std_sc_drc.csv", row.names=FALSE)
